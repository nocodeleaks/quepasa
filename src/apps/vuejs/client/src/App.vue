    ...
    <footer v-if="!isLoginPage" class="app-footer">
      <div class="footer-content">
        ...
        <div class="footer-flags">
          <span v-if="hasMasterKey" class="ftag ok" :title="masterFlagTitle">
            <i class="fa fa-key"></i>
            <span class="ftag-label">{{ t('masterkey_label') }}</span>
          </span>
          <span class="ftag" :class="sipFlagClass" :title="sipFlagTitle">
            <i class="fa fa-phone-volume"></i>
            <span class="ftag-label">{{ t('sip_proxy_label') }}</span>
            <span class="ftag-dot"></span>
          </span>
        </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const sipRunning = ref(false)

    const { t, locale } = useLocale()
    const { isDark } = useTheme()

    // Footer status flag helpers (MASTERKEY + SIP proxy)
    const masterFlagTitle = computed(() =>
      hasMasterKey.value ? t('masterkey_configured') : t('masterkey_not_configured')
    )
    const sipFlagClass = computed(() => {
      if (!sipConfigured.value) return 'off'
      return sipRunning.value ? 'ok' : 'warn'
    })
    const sipFlagTitle = computed(() => {
      if (!sipConfigured.value) return t('sip_proxy_not_configured')
      return sipRunning.value ? t('sip_proxy_running') : t('sip_proxy_configured_not_running')
    })

    // Query the SIP proxy state. Runs independently of session hydration so a
    // late-arriving user object does not skip it. Console logs let operators
    // follow each query and its parsed result in the browser devtools.
    async function fetchSipStatus() {
      console.debug('[sip-status] GET /api/sipproxy/status …')
      try {
        const res = await api.get('/api/sipproxy/status')
        console.debug('[sip-status] response', res.status, res.data)
        sipConfigured.value = Boolean(res.data?.configured)
        sipRunning.value = Boolean(res.data?.running)
        console.debug(
          `[sip-status] parsed -> configured=${sipConfigured.value} running=${sipRunning.value}` +
          ` host=${res.data?.host ?? ''} port=${res.data?.port ?? ''} protocol=${res.data?.protocol ?? ''}`
        )
      } catch (e: any) {
        console.warn('[sip-status] query failed:', e?.response?.status, e?.response?.data ?? e?.message)
      }
    }

    // Query the MASTERKEY state authoritatively, independent of session
    // hydration, so the footer flag does not flicker on/off during load.
    async function fetchMasterKeyStatus() {
      console.debug('[masterkey-status] GET /api/account/masterkey …')
      try {
        const res = await api.get('/api/account/masterkey')
        const configured = Boolean(res.data?.configured)
        console.debug('[masterkey-status] response', res.status, res.data, '-> configured=', configured)
        hasMasterKey.value = configured
      } catch (e: any) {
        console.warn('[masterkey-status] query failed:', e?.response?.status, e?.response?.data ?? e?.message)
      }
    }
    ...
    const loadBranding = async () => {
      try {
        ...
        // Load UI prefs after branding. MASTERKEY availability is resolved
        // authoritatively by fetchMasterKeyStatus (called on mount), so it is
        // NOT set here — doing so behind the session guard caused the footer
        // flag to flicker on/off when the session hydrated late.
...
/* ===== Footer status flags (MASTERKEY + SIP proxy) ===== */
.footer-flags {
  display: flex;
  align-items: center;
  gap: 0.45rem;
...
.ftag i { font-size: 0.72rem; }
.ftag-dot { width: 7px; height: 7px; border-radius: 50%; }
.ftag.ok   { background: rgba(220, 252, 231, 0.9); color: #15803d; border-color: rgba(34, 197, 94, 0.32); }
.ftag.ok   .ftag-dot { background: #22c55e; box-shadow: 0 0 0 3px rgba(34, 197, 94, 0.18); }
.ftag.warn { background: rgba(254, 243, 199, 0.9); color: #92400e; border-color: rgba(245, 158, 11, 0.32); }
.ftag.warn .ftag-dot { background: #f59e0b; box-shadow: 0 0 0 3px rgba(245, 158, 11, 0.18); }
.ftag.off  { background: rgba(241, 245, 249, 0.9); color: #64748b; border-color: rgba(148, 163, 184, 0.32); }
.ftag.off  .ftag-dot { background: #94a3b8; }

html[data-theme='dark'] .ftag.ok   { background: rgba(22, 101, 52, 0.32); color: #86efac; border-color: rgba(34, 197, 94, 0.4); }
html[data-theme='dark'] .ftag.warn { background: rgba(146, 64, 14, 0.32); color: #fcd34d; border-color: rgba(245, 158, 11, 0.4); }
html[data-theme='dark'] .ftag.off  { background: rgba(51, 65, 85, 0.6); color: #94a3b8; border-color: rgba(148, 163, 184, 0.3); }

@media (max-width: 640px) {
  .ftag-label { display: none; }
}
