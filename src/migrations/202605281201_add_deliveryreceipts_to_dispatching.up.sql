CREATE TABLE IF NOT EXISTS `dispatching_202605281201` (
...
INSERT INTO `dispatching_202605281201` (
  `context`, `connection_string`, `type`, `forwardinternal`, `trackid`,
  `readreceipts`, `deliveryreceipts`, `groups`, `broadcasts`, `calls`, `direct`,
  `extra`, `failure`, `success`, `timestamp`
)
SELECT
  `context`, `connection_string`, `type`, `forwardinternal`, `trackid`,
  `readreceipts`, 0, `groups`, `broadcasts`, `calls`, `direct`,
  `extra`, `failure`, `success`, `timestamp`
FROM `dispatching`;

DROP TABLE `dispatching`;
ALTER TABLE `dispatching_202605281201` RENAME TO `dispatching`;
