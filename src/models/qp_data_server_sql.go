...
func (source QpDataServerSql) FindForUser(token string, user string) (response *QpServer, err error) {
	response = &QpServer{}
	err = source.db.Get(response, "SELECT token, wid, verified, devel, groups, broadcasts, readreceipts, calls, readupdate, direct, user, metadata, timestamp FROM servers WHERE token = ? AND user = ?", token, user)
	if err != nil {
		response = nil
	}
	return
...
func (source QpDataServerSql) FindAll() (response []*QpServer) {
	_ = source.db.Select(&response, "SELECT token, wid, verified, devel, groups, broadcasts, readreceipts, calls, readupdate, direct, user, metadata, timestamp FROM servers")
	return
}
...
func (source QpDataServerSql) FindByToken(token string) (response *QpServer, err error) {
	response = &QpServer{}
	err = source.db.Get(response, "SELECT token, wid, verified, devel, groups, broadcasts, readreceipts, calls, readupdate, direct, user, metadata, timestamp FROM servers WHERE token = ?", token)
	if err != nil {
		response = nil
	}
	return
...
func (source QpDataServerSql) Add(element *QpServer) error {
	query := `INSERT INTO servers (
		token, wid, verified, devel, groups, broadcasts, readreceipts, calls, readupdate, direct, user, metadata
	) VALUES (
		:token, :wid, :verified, :devel, :groups, :broadcasts, :readreceipts, :calls, :readupdate, :direct, :user, :metadata
	)`
