CREATE TABLE IF NOT EXISTS `servers_202604221930` (
...
INSERT INTO `servers_202604221930`(
  `token`, `wid`, `verified`, `devel`, `metadata`,
  `groups`, `broadcasts`, `readreceipts`, `calls`, `readupdate`, `user`, `timestamp`
)
SELECT
  `token`, `wid`, `verified`, `devel`, NULL,
  `groups`, `broadcasts`, `readreceipts`, `calls`, `readupdate`, `user`, `timestamp`
FROM `servers`;

DROP TABLE `servers`;
ALTER TABLE `servers_202604221930` RENAME TO `servers`;
