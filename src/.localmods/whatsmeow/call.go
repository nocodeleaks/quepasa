...
func (cli *Client) handleCallEvent(ctx context.Context, node *waBinary.Node) {
	...
	cli.Log.Infof("[RAW-PROBE-UPSTREAM] handleCallEvent tag=%s xml=%s", node.Tag, node.XMLString())
	_ = os.MkdirAll(`.dist\call_dumps`, 0o755)
	_ = os.WriteFile(`.dist\call_dumps\raw_call_dispatch_probe.txt`, []byte(node.XMLString()), 0o644)
