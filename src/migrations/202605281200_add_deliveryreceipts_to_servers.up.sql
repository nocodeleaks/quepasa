CREATE TABLE IF NOT EXISTS `servers_202605281200` (
...
INSERT INTO `servers_202605281200`(
  `token`, `wid`, `verified`, `devel`, `metadata`,
  `groups`, `broadcasts`, `readreceipts`, `deliveryreceipts`, `calls`, `readupdate`, `direct`, `user`, `timestamp`
)
SELECT
  `token`, `wid`, `verified`, `devel`, `metadata`,
  `groups`, `broadcasts`, `readreceipts`, 0, `calls`, `readupdate`, `direct`, `user`, `timestamp`
FROM `servers`;

DROP TABLE `servers`;
ALTER TABLE `servers_202605281200` RENAME TO `servers`;
