...
//  1. If a whatsmeow device JID's phone number matches a server that was
//     previously linked to that phone (via Hermes inbox token), that server
//     is preferred.
//  2. If no phone-based match is found but there is exactly one orphan device
//     and exactly one unlinked server, they are linked automatically.
//  3. Devices and servers that cannot be matched unambiguously are listed in
//     the report but left unchanged — never force-linked.
//
...
func (source *QPWhatsappService) RestoreOrphaned() (*RestoreReport, error) {
	...
	// ── Build a phone → token index from unlinked servers ────────────────────
	// We store runtime server state (including the phone number derived from
	// the live WhatsApp connection). Servers that are already in the runtime
	// cache may expose their phone even without a wid stored in the DB.
	phoneToToken := make(map[string]string)
	for _, token := range report.UnlinkedServers {
		if rts, ok := source.Servers[token]; ok {
			phone := rts.GetNumber()
			if len(phone) > 0 {
				phoneToToken[phone] = token
			}
		}
	}

	// Track which unlinked tokens have been consumed so we can fall back to the
	// one-to-one rule without double-assigning.
	consumed := make(map[string]bool)

	// ── Pass 1: phone-based matching ─────────────────────────────────────────
	remaining := make([]WhatsmeowOrphanDevice, 0, len(report.OrphanDevices))
	for _, orphan := range report.OrphanDevices {
		token, matched := phoneToToken[orphan.Phone]
		if !matched || consumed[token] {
			remaining = append(remaining, orphan)
			continue
		}

		if restErr := source.linkAndRestore(token, orphan.JID); restErr != nil {
			logentry.Errorf("phone-match restore failed token=%s jid=%s: %v", token, orphan.JID, restErr)
			report.Errors = append(report.Errors, RestoreError{Token: token, JID: orphan.JID, Reason: restErr.Error()})
		} else {
			logentry.Infof("restored via phone match: token=%s jid=%s", token, orphan.JID)
			report.Restored = append(report.Restored, RestoreResult{Token: token, JID: orphan.JID})
			consumed[token] = true
		}
	}

	// ── Pass 2: one-to-one fallback ───────────────────────────────────────────
	// When exactly one orphan device and exactly one unlinked server remain,
	// the association is unambiguous and we apply it automatically.
	availableTokens := make([]string, 0, len(report.UnlinkedServers))
	for _, t := range report.UnlinkedServers {
		if !consumed[t] {
			availableTokens = append(availableTokens, t)
		}
	}

	if len(remaining) == 1 && len(availableTokens) == 1 {
		orphan := remaining[0]
		...
		// Multiple orphans with multiple unlinked servers — ambiguous.
		// Log and surface them but do not auto-link.
