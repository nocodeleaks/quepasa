...
func (source *SIPCallManagerSipgo) CreateSDPOffer(callID, fromPhone string) (string, error) {
	...
	// Advertise the EXACT local UDP port the audio bridge listens on for this
	// call (registered via SetLocalRTPPort). The SIP server sends its RTP to
	// this port and we send the return direction from the same socket
	// (symmetric RTP). Without a registered port there is no valid socket to
	// advertise, so fail loudly rather than offering a bogus port that would
	// silently break two-way audio.
	v, ok := source.localRTPPorts.Load(callID)
	if !ok {
		return "", fmt.Errorf("CreateSDPOffer: no registered local RTP port for CallID %s (SetLocalRTPPort must run before INVITE)", callID)
	}
	rtpPort := int64(v.(int))
	...
	// Offer ONLY L16/16000 (payload type 118): full-rate 16 kHz linear PCM, the
	// same rate the WhatsApp MLow codec decodes to, so the QuePasa↔asterisk leg
	// is lossless and asterisk handles transcoding to the destination codec.
	// The audio bridge encodes/decodes L16 exclusively, so no μ-law is offered.
	return fmt.Sprintf(`v=0
o=%s %d %d IN IP4 %s
s=%s
c=IN IP4 %s
t=0 0
m=audio %d RTP/AVP 118 101
