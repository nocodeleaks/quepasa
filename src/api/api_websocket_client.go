...
func (c *WebSocketClient) readPump(pairing models.QpWhatsappPairing) {
	defer func() {
		c.hub.unregister <- c
	}()
...
func (c *WebSocketClient) QrCodeScanner(pairing models.QpWhatsappPairing) {
	out := make(chan []byte)
	defer close(out)
	go func() {
...
// WebSocketStart handles websocket requests from the peer.
func WebSocketStart(pairing models.QpWhatsappPairing, conn *websocket.Conn) {
	...
	client := &WebSocketClient{hub: hub, conn: conn, send: make(chan []byte, 256), ctx: execContext}
...
func (c *WebSocketClient) OnWebSocketClosed(code int, text string) error {
	c.Cancel()
