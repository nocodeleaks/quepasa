...
func dumpRawSentCallLikeNode(node waBinary.Node) {
    if node.Tag != "call" && node.Tag != "receipt" && node.Tag != "ack" {
        return
    }
    ...
    if node.Tag == "call" {
        if raw, ok := node.Attrs["call-id"]; ok {
        ...
        if len(children) > 0 {
            ...
            subtag = child.Tag
            if callID == "nocallid" {
                if raw, ok := child.Attrs["call-id"]; ok {
                    if s := strings.TrimSpace(fmt.Sprint(raw)); s != "" {
                        callID = s
                    }
                }
    ...
    } else if node.Tag == "receipt" && len(children) > 0 {
        child := children[0]
        subtag = child.Tag
        if raw, ok := child.Attrs["call-id"]; ok {
            if s := strings.TrimSpace(fmt.Sprint(raw)); s != "" {
		...
		ackType := ""
		if raw, ok := node.Attrs["type"]; ok {
			if s := strings.TrimSpace(fmt.Sprint(raw)); s != "" {
				ackType = s
			}
		}
		if raw, ok := node.Attrs["class"]; ok {
			if s := strings.TrimSpace(fmt.Sprint(raw)); s != "" {
				subtag = s
			}
		}
		if len(children) > 0 {
			...
			if ackType != "" && child.Tag != "" && ackType != child.Tag {
				subtag = ackType + "_" + child.Tag
			} else if ackType != "" && subtag == "none" {
				subtag = ackType
			}
		} else if ackType != "" {
			subtag = ackType
    ...
    dir := filepath.Join(".dist", "call_dumps")
    _ = os.MkdirAll(dir, 0o755)
    path := filepath.Join(dir, fmt.Sprintf("%s_raw_sent_%d_%s_%s.xml", node.Tag, time.Now().UnixNano(), callID, subtag))
    _ = os.WriteFile(path, []byte(node.XMLString()), 0o644)
}
