- ✅ Session auth fluidity slice: `POST /api/sessions` now always requires user identity via SPA JWT (`Authorization`) or `X-QUEPASA-TOKEN`; `RELAXED_SESSIONS` controls only whether master key is additionally required; removed master-key-only default-user fallback; canonical protected middleware now accepts `X-QUEPASA-TOKEN` for JWT verification; added canonical tests for header-token success, strict-mode 403, and master-key-only 401
